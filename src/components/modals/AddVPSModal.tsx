...
export default function AddVPSModal({ isOpen, closeModal, editForm = false, id = 0, addVpsToState }: PropsType) {
  ...
  useEffect(() => {
    if (id > 0) {
      const fetchVPS = async () => {
        try {
          const res = await fetch(import.meta.env.VITE_API_URL + "/api/vps/" + id, {
            method: "GET",
            headers: {
  ...
  const checkVPSConnection = async (data: VPSDto) => {
    setChecking(true);
    await fetch(import.meta.env.VITE_API_URL + "/api/vps/check-connection", {
      method: "POST",
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}` // <-- tu wysyłamy token
  ...
  const updateVPS = async (data: VPSDto) => {
    setChecking(true);
    try {
      const response = await fetch(import.meta.env.VITE_API_URL + "/api/vps/" + id, {
        method: "PUT",
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}` // <-- tu wysyłamy token
  ...
  const addVPS = async (data: VPSDto) => {
    setChecking(true);
    try {
      const response = await fetch(import.meta.env.VITE_API_URL + "/api/vps", {
        method: "POST",
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}` // <-- tu wysyłamy token
