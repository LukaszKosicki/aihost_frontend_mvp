...
export default function AddAIModelModal({ isOpen, closeModal, selectedEvent = false, id }: PropsType) {
  ...
  useEffect(() => {
    const fetchModel = async () => {
      try {
        const res = await fetch(import.meta.env.VITE_API_URL + `/api/adminmodels/${id}`, {
          method: "GET",
          headers: {
  ...
  const addAIModel = async () => (
    id ?
    await fetch(import.meta.env.VITE_API_URL + `/api/adminmodels/${id}`, {
      method: "PUT",
      headers: {
   ...
   //   return response.json(); // lub response.text(), zależnie od API
      ...
      await fetch(import.meta.env.VITE_API_URL + "/api/adminmodels", {
      method: "POST",
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
