...
export default function ContainerActions({ containerId, vpsId, changeContainerState, removeContainer }: PropsType) {
  ...
  const containerStop = async () => {
    setIsOpen(false);
    setIsLoading(true);
    await fetch(import.meta.env.VITE_API_URL + "/api/container/stop", {
      method: "POST",
      headers: {
  ...
  const containerStart = async () => {
      setIsOpen(false);
    setIsLoading(true);
    await fetch(import.meta.env.VITE_API_URL + "/api/container/start", {
      method: "POST",
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
  ...
  const containerDelete = async () => {
      setIsOpen(false);
      setIsLoading(true);
    await fetch(import.meta.env.VITE_API_URL + `/api/container/delete/${vpsId}/${containerId}`, {
      method: "DELETE",
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
